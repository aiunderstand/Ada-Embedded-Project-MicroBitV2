...
def cmd_extension(args) -> int:
    """Package the VS Code web extension, and optionally install it.

    A .vsix is a zip with a manifest, so this needs no node, npm or vsce -- none
    of which are in the container. That matters: the extension can be built and
    installed inside a Codespace with nothing but Python.
    """
    import zipfile
    ...
    # The version is derived from the content. VS Code Server serves extension
    # files with "Cache-Control: public, max-age=31536000" at a URL that
    # contains only <publisher>.<name>-<version>, so reinstalling changed code
    # under the same version leaves the browser running last week's
    # extension.js after a reload -- the manifest arrives over RPC and is
    # fresh, the code comes from the HTTP cache and is not. A new version is
    # a new directory and a new URL. major.minor stay as written in
    # extension/package.json; the patch number is the first six hex digits of
    # a hash of everything that goes into the package.
    major, minor = pkg["version"].split(".")[:2]
...
""")
    ...
    BUILD.mkdir(parents=True, exist_ok=True)
    # One package in build/: the name carries the version, so a stale one
    # from an earlier source would otherwise sit next to it, installable.
    for stale in BUILD.glob(f"{name}-*.vsix"):
        stale.unlink()
    ...
    if args.install:
        code_cli = shutil.which("code") or shutil.which("code-insiders")
        if not code_cli:
        ...
        installed = _installed_extension_dirs(publisher, name)
        current = [d for d in installed if d.name.endswith(f"-{version}")
                   and _installed_matches(d, bundled, pkg_text)]
        if current:
            info(f"already installed and up to date ({rel(current[0])})")
            return 0
        if run([code_cli, "--install-extension", str(vsix), "--force"]) != 0:
            die("installing the extension failed")
        if installed:
            print(f"  updated the extension to {version}. VS Code keeps the old one")
            print("  running until the window is reloaded:")
...
def _installed_extension_dirs(publisher: str, name: str) -> list[Path]:
    """Every folder VS Code has unpacked our extension into here, any version.
    ...
    for root in roots:
        if not root.is_dir():
            continue
        # VS Code does not delete a superseded version at once: it lists the
        # folder in .obsolete and removes it later. Such a folder is not
        # installed, however identical its content is.
        try:
            obsolete = set(json.loads((root / ".obsolete").read_text()))
        except (OSError, ValueError):
            obsolete = set()
        found = sorted(d for d in root.iterdir()
                       if d.is_dir() and d.name.lower().startswith(prefix)
                       and d.name not in obsolete)
        if found:
            return found
    return []
...
def main() -> int:
    ...
    p = sub.add_parser("extension",
                       help="package the VS Code web extension (and --install it)")
    p.add_argument("--install", action="store_true",
                   help="install it into this VS Code after packaging")
